...
bool parse_output(const InferenceEngine::Blob::Ptr &blob, int scale, float conf_threshold, std::vector<cv::Rect> &o_rect,std::vector<float> &o_rect_conf, std::vector<std::string> &lbls) {
    ...
    int item_size = 11;   // total_class + 5
    size_t anchor_n = 3;
    for (int n = 0; n < anchor_n; ++n)
        for (int i = 0; i < scale; ++i)
            for (int j = 0 ; j < scale; ++j) {
                ...
                std::string lbl = "";
                // if (idx == 0) 
                //     lbl = "DL2CAW9699";  //["AAN","hERO","434"]  ["MP15N","C9783","MP15","MG4730","UP15CS0090",DL2CAW9699]
                ...
                // add the boxes and label to the global list
...
void pred(cv::Mat img, InferenceEngine::ExecutableNetwork exec_net, double conf_threshold, double nms_threshold, std::vector<Object> &detectedObjects) {
    ...
    for (auto &output: output_info) {
        ...
        parse_output(blob, scales[i], conf_threshold, origin_rect, origin_rect_conf, lbls);
        ++i;
    }
    ...
    for (int i = 0; i < final_id.size(); ++i) {
        ...
        std::cout<<"lbls[final_id[i]]: "<< lbls[final_id[i]]<<std::endl;
    }
}

int main (int argc, char const *argv[]) {
    const std::string model_path = "/home/sensovision/Desktop/openvino_inferencing code/Openvino/openvino python inference/detection/ocr_ifb/best_ifb_ocr.xml";
    ...
    cv::Mat img = cv::imread("/home/sensovision/Desktop/openvino_inferencing code/Openvino/openvino python inference/detection/ocr_ifb/1.jpg");
    ...
    pred(img, exec_net, 0.5, 0.5, detectedObjects);  //confidence_score and iou threshold
    ...
    for (int i = 0; i < detectedObjects.size(); ++i) {
        ...
        cv::putText(clone,lbl,cv::Point(xmin,ymin-5),cv::FONT_HERSHEY_DUPLEX,1,cv::Scalar(0,255,0),2,false);
    ...
    std::cout << "[INFO] given image is " << lbl << std::endl;
