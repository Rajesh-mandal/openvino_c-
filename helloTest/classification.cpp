...
int pred(cv::Mat img, InferenceEngine::ExecutableNetwork exec_net, double threshold) {
    ...
    img = cv::dnn::blobFromImage(img, 1.0, cv::Size(300, 300), cv::Scalar(0));
    ...
    InferenceEngine::TensorDesc desc(InferenceEngine::Precision::FP32, {1, 3, 300, 300}, InferenceEngine::Layout::NCHW);
    ...
    for(auto &item : output_info) {
        ...
        cout<<"buffer: "<<buffer[10]<<endl;
        // extract the pixel values from the image
        float* img_data = (float *)img.data;
        cout<<"image data[0]:"<<img_data[10]<<endl;
        cout<<"img_data:"<<img_data<<endl;
        // compute the total number of pixels
        int total_pixels = 300 * 300 * 3;        

        // loop through the image, pixel by pixel and compute the error
        double error = 0.0;
        for(int i = 0; i < total_pixels; ++i)
        
            error += pow(((img_data[i] / 255.0) - buffer[i]), 2);
            cout<<"error inside for loop:"<<error<<endl; 

        // compute the mean error
        cout<<"before error: "<<error<<endl;

        error = error / total_pixels;
        cout<<"error: "<<error<<endl;
        // threshold the error, to compute the label
        lbl = (error < threshold) ? 0 : 1;
    }
...
int main() {
    ...
    cv::Mat img = cv::imread("/home/sensovision/Desktop/Rajesh/Rajesh/datasets/connect_well/C7S4UN/ok/15-11-2021_04_14_27img_361.jpg");

    // call the prediction function
    std::clock_t start = std::clock();
    int lbl = pred(img, net, 0.99);
